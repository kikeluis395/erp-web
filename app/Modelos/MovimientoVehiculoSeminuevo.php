...
class MovimientoVehiculoSeminuevo extends Model
{
    protected $table="movimiento_vehiculo_seminuevo";
    protected $fillable=['id_vehiculo_seminuevo', 'tipo_movimiento', 'cantidad_movimiento', 'fecha_movimiento', 'id_local_empresa','saldo','fuente_type','fuente_id','costo', 'costo_promedio_ingreso','saldo_dolares'];
    protected $primaryKey='id_movimiento_vehiculo_seminuevo';
    ...
    private static function generarMovimiento($idVehiculoNuevo, $idLocal, $cantidad, $tipoMovimiento,  $tipo_fuente, $fuente_id)
    {
        ...
        if($puedeGenerar){
            try{
                ...
                $movimiento = MovimientoVehiculoSeminuevo::create([
    ...
    private static function getCosto($id_vehiculo_nuevo){
        ...
        $listLastMovimiento = MovimientoVehiculoSeminuevo::where('id_vehiculo_nuevo', $id_vehiculo_nuevo)->orderBy('fecha_movimiento','desc')->where('tipo_movimiento',"!=",'EGRESO VIRTUAL')->get();
    ...
    private static function miSaldoFisico($id_vehiculo_nuevo){
        $lastMovimiento = MovimientoVehiculoSeminuevo::where('id_vehiculo_nuevo', $id_vehiculo_nuevo)->where('tipo_movimiento',"!=",'EGRESO VIRTUAL')->orderBy('fecha_movimiento','desc')->first();
