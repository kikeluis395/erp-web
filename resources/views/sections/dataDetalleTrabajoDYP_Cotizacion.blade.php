<div class="row col-12 px-0 my-3">
    ...
    <div class="row col-sm-6 col-lg-4 align-content-start">
        <span class="font-weight-bold letra-rotulo-detalle col-6 text-right"
              style="height: fit-content">COTIZACIÓN: </span>
        <span class="col-6   " style="height: fit-content">{{ $datosHojaTrabajo->id_cotizacion }}</span>
        {{-- <br> --}}
        <span class="font-weight-bold letra-rotulo-detalle col-6 text-right"
              style="height: fit-content">PLACA: </span>
        <span class="col-6   " style="height: fit-content">
        ...
        {{-- <br> --}}
        <span class="font-weight-bold letra-rotulo-detalle col-6 text-right"
              style="height: fit-content">MARCA: </span>
        <span id="marcaVehiculo"
              class="col-6   " style="height: fit-content">{{ $datosHojaTrabajo->vehiculo->getNombreMarca() }}</span>
        {{-- <br> --}}
        <span class="font-weight-bold letra-rotulo-detalle col-6 text-right"
              style="height: fit-content">MODELO TECNICO: </span>
        <span id="modeloTecnicoVehiculo"
              class="col-6   " style="height: fit-content">{{ $datosHojaTrabajo->vehiculo->getNombreModeloTecnico() }}</span>
        {{-- <br> --}}
        <span class="font-weight-bold letra-rotulo-detalle col-6 text-right"
              style="height: fit-content">VIN: </span>
        <span id="vinVehiculo"
              class="col-6   " style="height: fit-content">{{ $datosHojaTrabajo->vehiculo->getVin() }}</span>
    </div>
                ...
                <select name="seguroSelect"
                    ...
                    @foreach ($listaSeguros as $seguro)
                        <option value="{{ $seguro->id_cia_seguro }}"
                                @if ($nombreCiaSeguro == $seguro->nombre_cia_seguro) selected @endif>{{ $seguro->nombre_cia_seguro }}</option>
                    @endforeach
                </select>
            @else
                {{ $nombreCiaSeguro }}
